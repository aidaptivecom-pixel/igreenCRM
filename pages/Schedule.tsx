...
const getStatusColor = (status: AppointmentStatus) => {
  switch(status) {
    ...
    case 'Completado': return 'bg-gray-500/20 border-gray-500/50 text-gray-400';
    case 'NoShow': return 'bg-purple-500/20 border-purple-500/50 text-purple-400';
...
export const Schedule: React.FC = () => {
  ...
  const handlePrevDay = () => {
    // Mock logic to switch between 4 and 5 dec
    ...
    <div className="h-[calc(100vh-140px)] flex flex-col gap-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-4">
             <button onClick={handlePrevDay} className="p-2 hover:bg-white/10 rounded-full transition-colors"><ChevronLeft size={20} /></button>
             ...
             <button onClick={handleNextDay} className="p-2 hover:bg-white/10 rounded-full transition-colors"><ChevronRight size={20} /></button>
             <button className="text-sm text-green-400 hover:text-green-300 font-medium">Hoy</button>
          </div>
        </div>
      ...
      <div className="flex-1 flex gap-6 overflow-hidden">
        ...
        <div className="flex-1 glass-card p-0 flex flex-col overflow-hidden relative">
           ...
           <div className="h-12 border-b border-white/10 flex items-center px-6">
              <span className="text-sm text-gray-400 font-medium">Horario</span>
              <div className="ml-16 border-l border-white/10 pl-4 h-full flex items-center">
           ...
           <div className="flex-1 overflow-y-auto custom-scrollbar relative bg-black/20">
             ...
             <div className="absolute top-0 left-0 right-0" style={{ height: TOTAL_HOURS * PIXELS_PER_HOUR }}>
               {Array.from({ length: TOTAL_HOURS }).map((_, i) => (
                 <div key={i} className="flex border-b border-white/5 h-[120px] group hover:bg-white/[0.02] transition-colors">
                    <div className="w-20 shrink-0 border-r border-white/10 p-2 text-xs text-gray-500 font-mono text-right relative">
                       <span className="-top-3 relative block">{START_HOUR + i}:00</span>
                    </div>
                    <div className="flex-1 relative">
                      {/* Half hour line */}
                      <div className="absolute top-1/2 left-0 right-0 border-t border-white/5 border-dashed w-full h-px opacity-30"></div>
                    </div>
                 </div>
               ))}
               {/* Final Time Label */}
                <div className="w-20 border-r border-white/10 p-2 text-xs text-gray-500 font-mono text-right relative h-0">
        ...
        <div className="w-80 flex flex-col gap-6 shrink-0">
          ...
          <GlassCard className="p-4 space-y-4">
            <h3 className="text-sm font-semibold text-gray-400 uppercase tracking-wider mb-2">Resumen del Día</h3>
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="bg-green-500/10 rounded-xl p-3 text-center">
                <span className="block text-2xl font-bold text-green-400">{stats.confirmed}</span>
                <span className="text-xs text-green-500/70">Confirmados</span>
              </div>
              <div className="bg-yellow-500/10 rounded-xl p-3 text-center">
                <span className="block text-2xl font-bold text-yellow-400">{stats.pending}</span>
                <span className="text-xs text-yellow-500/70">Pendientes</span>
              </div>
              <div className="bg-gray-500/10 rounded-xl p-3 text-center">
                <span className="block text-2xl font-bold text-gray-400">{stats.completed}</span>
             ...
             <div className="flex-1 overflow-y-auto p-4 space-y-3">
                {dayAppointments.length > 0 ? (
                  dayAppointments
                    ...
                    .map((apt) => (
                      <div key={apt.id} className="p-3 rounded-xl bg-white/5 border border-white/5 hover:bg-white/10 transition-colors cursor-pointer group">
                        <div className="flex justify-between items-start mb-1">
                          <span className="font-medium text-sm">{apt.clientName}</span>
                          <span className="text-xs text-gray-400 bg-black/20 px-1.5 py-0.5 rounded">{apt.time}</span>
                        </div>
                        ...
                        <div className="flex items-center justify-between">
                          <span className={`text-[10px] px-1.5 py-0.5 rounded border ${
                            apt.status === 'Confirmado' ? 'border-green-500/30 text-green-400' :
                            apt.status === 'Pendiente' ? 'border-yellow-500/30 text-yellow-400' :
          ...
          <GlassCard className="w-full max-w-lg bg-[#141414] border-white/20 animate-in slide-up">
            ...
            <form className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs text-gray-400 mb-1 ml-1">Fecha</label>
                  <div className="relative">
                ...
                <div>
                  <label className="block text-xs text-gray-400 mb-1 ml-1">Hora</label>
                  <div className="relative">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs text-gray-400 mb-1 ml-1">Tipo</label>
                ...
                <div>
                   <label className="block text-xs text-gray-400 mb-1 ml-1">Duración</label>
                   <select className="glass-input w-full">
              ...
              <div>
                <label className="block text-xs text-gray-400 mb-1 ml-1">Dispositivo & Servicio</label>
                <div className="relative">
      ...
      {selectedAppointment && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
           <GlassCard className="w-full max-w-md bg-[#141414] border-white/20 animate-in slide-up relative">
              <button 
              ...
              <div className="mb-6">
                <div className="flex items-center gap-2 mb-1">
                   ...
                   <h3 className="text-xl font-bold">{selectedAppointment.type}</h3>
                </div>
                <p className="text-gray-400">{selectedAppointment.service}</p>
              </div>
              ...
              <div className="space-y-4 mb-6">
                ...
                <div className="flex items-center gap-3">
                   ...
                   <p className="font-medium">{selectedAppointment.device}</p>
                </div>
                <div className="flex items-center gap-3">
                   <Clock className="text-gray-500" size={18} />
                   <p className="font-medium">{selectedAppointment.time} <span className="text-gray-500 font-normal">({selectedAppointment.duration} min)</span></p>
                </div>
