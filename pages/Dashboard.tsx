      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {KPIS.map((kpi, idx) => (
          <GlassCard key={idx} className="relative overflow-hidden group">
            <div className="absolute right-0 top-0 p-32 bg-green-500/5 rounded-full blur-3xl group-hover:bg-green-500/10 transition-all" />
            <div className="flex justify-between items-start mb-4 relative z-10">
              <div className="p-3 bg-white/5 rounded-2xl border border-white/5">
                {kpi.icon === 'dollar' && <DollarSign size={24} className="text-green-400" />}
                {kpi.icon === 'tool' && <PenTool size={24} className="text-blue-400" />}
                {kpi.icon === 'calendar' && <Calendar size={24} className="text-purple-400" />}
                {kpi.icon === 'message' && <MessageCircle size={24} className="text-orange-400" />}
              </div>
              <div className={`flex items-center gap-1 text-xs font-medium px-2 py-1 rounded-full ${kpi.trendUp ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400'}`}>
            ...
            <div className="h-[300px] w-full">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={chartData}>
                  ...
                  />
                  <Bar dataKey="ventas" fill="#10B981" radius={[4, 4, 0, 0]} stackId="a" />
                  <Bar dataKey="reparaciones" fill="#3B82F6" radius={[4, 4, 0, 0]} stackId="a" />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </GlassCard>
        ...
        <div className="space-y-6">
          <GlassCard>
            <h2 className="text-lg font-semibold mb-4 flex items-center gap-2">
              <AlertTriangle className="text-yellow-500" size={20} /> Alertas
            </h2>
            <div className="space-y-3">
              {[
                ...
                <div key={i} className="flex items-center gap-3 p-3 rounded-xl bg-white/5 hover:bg-white/10 transition-colors cursor-pointer border border-transparent hover:border-white/5">
             ...
             <div className="space-y-3">
              {[
                ...
                <div key={i} className="flex items-center gap-3 p-2 group">
                  <div className={`w-5 h-5 rounded-md border flex items-center justify-center transition-colors ${task.done ? 'bg-green-500 border-green-500' : 'border-gray-600 group-hover:border-green-500'}`}>
                    {task.done && <CheckCircle2 size={14} className="text-black" />}
                  </div>
                  <span className={`text-sm ${task.done ? 'text-gray-500 line-through' : 'text-gray-300'}`}>{task.msg}</span>
                </div>
              ))}
            </div>
          </GlassCard>
